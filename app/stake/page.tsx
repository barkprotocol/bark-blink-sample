  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw new Error(result.error || 'An unknown error occurred while staking tokens.');
      }
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-lg">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white dark:bg-gray-900 p-8 rounded-lg shadow-lg">
        ...
        <button
          type="submit"
          className={`w-full px-4 py-2 rounded-md shadow-sm border border-transparent focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition-colors ${
            staking
              ? 'bg-gray-400 text-gray-300 cursor-not-allowed'
              : 'bg-primary text-black hover:bg-primary-dark dark:bg-primary-dark dark:text-white dark:hover:bg-primary'
          }`}
          disabled={staking}
          aria-busy={staking}
        >
        ...
        <div className="mt-6 text-center">
          ...
          <p>
            ...
            <a 
              ...
              aria-label={`View transaction ${stakeResult} on Solscan`}
            >
              {stakeResult}
            </a>
          </p>
